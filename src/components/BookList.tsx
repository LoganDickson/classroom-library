...
export default function BookList({ refreshKey }: Props) {
  ...
  return (
    <div>
      <p className="text-sm text-gray-500 mb-4">{books.length} books</p>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {books.map((book) => (
          <div
            key={book.id}
            className="bg-white rounded-lg shadow-sm border border-gray-200 p-3 flex flex-col items-center text-center"
          >
            {book.cover_url ? (
              <Image
                src={book.cover_url}
